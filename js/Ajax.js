...
$(function () {
    ...
    */
    ...
    for (var i = count - 1; i >= 0; i--) {
        ...
        //主导航的匹配链接
        var $a = $(".nav a[href*='" + current + "']");

        //主导航的匹配文字  [一般情况下是没有用的,有时候会碰到说是这个文字,但是链接确是另外一个,但是导航选中还是要加在文字上面]
        //比如 二级分类是成功案例,但是它下面还有一个三级分类是经典案例,主导航上的是二级分类确标着三级分类的链接,这个文字就有用了
        //况且这里还要先判断文字再判断链接

        var $t = $(".nav a:contains('" + txt + "')");


        //最后一次循环txt是首页,往往主导航的第一个都是**首页,所以要判断一下
        //判断文字
        if ($t.size() && txt != "首配套产品页") {
    ...
    if (!$(".nav li.cur").size()) {
        ...
        $(".nav a[href*='" + MARK + "']").parent("li").addClass("cur").siblings("li").removeClass("cur");
        //如果还没有主导航选中,给第一个加样式
        if (!$(".nav li.cur").size()) {
...
function case_liuy(src) {
    ...
    if (reg.test(textName) || textName == "请输入您的姓名") {
        err += "请输入您的姓名<br/>";
    }
    if (reg.test(textTel) || textTel == "请输入您的电话") {
        err += "请输入您的电话<br/>";
    ...
    if (textContent == "工程概况描述") {
        textContent = "";
    }
    showProc(src);
    $.post("/ajax.ashx?action=CaseLiuy&t=" + Math.random(), {
        name: textName,
        tel: textTel,
        content: textContent
    }, function (msg) {
...
function headNavPosition(MARK, SID) {
    ...
    if (MARK == "product") {
        if (SID.indexOf("0001,0041,0042") < 0 && SID.indexOf("0001,0041,0043") < 0 && SID.indexOf("0001,0041,0044") && SID.indexOf("0001,0041,0045")) {
            SID = "0001";
        }
        else {
    ...
    if (MARK == "help" && SID != "0001,0029" && SID != "0001,0030" && SID != "0001,0031" && SID != "0001,0016"  && SID != "0001,0010" && SID != "0001,0002") {
        if (SID.length > 8) {
            SID = SID.substring(0, 9);
        }
    }
